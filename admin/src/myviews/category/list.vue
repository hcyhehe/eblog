<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :key="tableKey" :data="list" border fit stripe highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="分类" width="" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  methods:{
    ...
    addSave(){      
      ...
      aPost(base.categoryAdd, this.params2).then(res=>{
        console.log('categoryAdd', res.data)
    ...
    getList(){
      let that = this
      aGet(base.categoryList, this.params).then(res=>{
    ...
    editSave(){
      ...
      aPost(base.categoryEdit, this.params2).then(res=>{
        console.log('categoryEdit', res.data)
        if(res.data.code==2000000){
    ...
    del(id){
      ...
      that.$confirm('此操作将删除该分类, 是否继续?', '提示', { 
        ...
        aPost(base.categoryDel, {id:id}).then(res=>{
          if(res.data.code==2000000){
